...
# --- App Configuration ---
APP_VERSION = "2.5.1" # Updated version: Hide inputs when viewing saved
...
# --- URL Input Section (Show only when not viewing saved) ---
# Check if we are viewing a specific saved comparison directly from query params
# because viewing_saved_id might be set later in the main flow
viewing_saved_id_check = st.query_params.get("view_id", [None])[0]

process_button = False # Default value
if not viewing_saved_id_check: # Only show inputs if NOT viewing a saved comparison
...
# (Keep Database functions as they were in the previous version)
@st.cache_resource
...
def get_db_connection():
    ...
    try: conn = psycopg2.connect(db_url, sslmode='require'); return conn
    except psycopg2.OperationalError as e: st.error(f"Database Connection Error: Could not connect. Check secrets/credentials. Details: {e}"); return None
    except Exception as e: st.error(f"Unexpected Database Connection Error: {e}"); return None

def init_db():
    conn = get_db_connection();
    if conn is None: st.error("DB Initialization failed: Could not connect."); return
    try:
        with conn.cursor() as cur:
            ...
            """)
        conn.commit()
    except Exception as e: st.error(f"Fatal DB Init Error: {e}"); conn.rollback()
    finally:
        if conn: conn.close()
...
def save_comparison(ounass_url, levelshoes_url, df_comparison):
    ...
    try:
        timestamp = datetime.now(); df_to_save = df_comparison.copy()
    ...
    except Exception as e: st.error(f"Database Error: Could not save comparison - {e}"); conn.rollback(); return False
    finally:
        if conn: conn.close()
...
def load_saved_comparisons_meta():
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            comparisons = cur.fetchall(); comparisons_list = [dict(row) for row in comparisons] if comparisons else []
    except Exception as e: st.error(f"Database Error: Could not load saved comparisons - {e}")
    finally:
        if conn: conn.close()
...
def load_specific_comparison(comp_id):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            cur.execute(sql, (comp_id,)); comp = cur.fetchone()
            if comp:
                comp_dict = dict(comp); fallback_name = f"ID {comp_dict['id']} ({comp_dict['timestamp']})"
                ...
                elif isinstance(json_data, list) or isinstance(json_data, dict) : df = pd.DataFrame(json_data)
                ...
                if not df.empty:
                    ...
                    if 'Display_Brand' not in df.columns:
                        ...
                        df['Display_Brand'] = np.where(df.get('Ounass_Count', 0) > 0, df[brand_ounass_col] if brand_ounass_col else df.get(brand_cleaned_col), df[brand_ls_col] if brand_ls_col else df.get(brand_cleaned_col)); df['Display_Brand'].fillna("Unknown", inplace=True)
...
def delete_comparison(comp_id):
    ...
    success = False
    try:
        with conn.cursor() as cur: sql = "DELETE FROM comparisons WHERE id = %s"; cur.execute(sql, (comp_id,))
        conn.commit(); success = True
    except Exception as e: st.error(f"Database Error: Could not delete comparison ID {comp_id} - {e}"); conn.rollback()
    finally:
        if conn: conn.close()
    return success
                ...
                for item in items:
                    try:
                        ...
                        if name_span:
                            ...
                            temp_name_span = BeautifulSoup(str(name_span), 'html.parser').find(class_='FacetLink-name'); temp_count_span = temp_name_span.find(class_='FacetLink-count')
...
def process_levelshoes_html(html_content):
    ...
    try:
        ...
        designer_facet = None
        for facet in facets: facet_key = facet.get('key', '').lower(); facet_label = facet.get('label', '').lower();
        if facet_key == 'brand' or facet_label == 'designer': designer_facet = facet; break
        if not designer_facet: available_facets = [f.get('key') or f.get('label') for f in facets]; st.error(f"Level Shoes Error: 'brand' or 'Designer' facet not found. Available: {available_facets}"); return data_extracted
        ...
        for option in designer_options: name = option.get('name'); count = option.get('count')
        if name is not None and count is not None: upper_name = name.upper();
        if "VIEW ALL" not in upper_name and "SHOW M" not in upper_name and "SHOW L" not in upper_name: data_extracted.append({'Brand': name.strip(), 'Count': int(count)})
        if not data_extracted: st.warning("Level Shoes: Designer options processed, but no valid brand data was extracted.")
        return data_extracted
    except json.JSONDecodeError: st.error("Level Shoes Error: Failed to decode JSON data from __NEXT_DATA__."); return []
    except (AttributeError, KeyError, TypeError, IndexError) as e: st.error(f"Level Shoes Error: Problem navigating JSON structure - {e}."); return []
    except Exception as e: st.error(f"Level Shoes Error: Unexpected error during processing - {e}"); return []

# --- Function to fetch HTML content from URL ---
# (Keep as is)
def fetch_html_content(url):
    if not url: st.error("Fetch error: URL cannot be empty."); return None
    try: headers = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36', 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8', 'Accept-Language': 'en-US,en;q=0.9', 'Connection': 'keep-alive' }; response = requests.get(url, headers=headers, timeout=30); response.raise_for_status(); return response.text
...
def ensure_ounass_full_list_parameter(url):
    ...
    try: parsed_url = urlparse(url); query_params = parse_qs(parsed_url.query, keep_blank_values=True); needs_update = param_key not in query_params or not query_params[param_key] or query_params[param_key][0] != param_value;
    if needs_update: query_params[param_key] = [param_value]; new_query_string = urlencode(query_params, doseq=True); url_components = list(parsed_url); url_components[4] = new_query_string; return urlunparse(url_components)
    else: return url
...
def extract_info_from_url(url):
    try:
        if not url: return None, None; parsed = urlparse(url); ignore_segments = ['ae', 'com', 'en', 'shop', 'category', 'all', 'view-all', 'plp', 'sale']; path_segments = [s for s in parsed.path.lower().split('/') if s and s not in ignore_segments];
        if not path_segments: return None, None; gender_keywords = ["women", "men", "kids", "unisex"]; gender = None; category_parts_raw = [];
        ...
        for part in category_parts_raw:
            if gender and part in gender_keywords: continue; cleaned = part.replace('.html', '').replace('-', ' ').strip();
            if cleaned: cleaned_category_parts.append(' '.join(word.capitalize() for word in cleaned.split()));
        category = " > ".join(cleaned_category_parts) if cleaned_category_parts else None; return gender, category
...
# Use viewing_saved_id_check defined earlier for consistency
# viewing_saved_id_sidebar = st.query_params.get("view_id", [None])[0] # Check directly
# (The rest of the sidebar logic using load_saved_comparisons_meta remains the same)
saved_comps_meta = load_saved_comparisons_meta()
if viewing_saved_id_check and st.session_state.get('confirm_delete_id') != viewing_saved_id_check : # Use checked var
...
if not saved_comps_meta: st.sidebar.caption("No comparisons saved yet.")
    ...
    st.sidebar.caption("Select two snapshots from the same group to compare over time."); url_group_keys = list(grouped_comps.keys())
    if 'selected_url_key_for_time_comp' not in st.session_state: st.session_state.selected_url_key_for_time_comp = None
    for idx, url_key in enumerate(url_group_keys):
        ...
        with st.sidebar.expander(expander_label, expanded=is_expanded):
            comp_options = {};
            for comp in sorted(comps_list, key=lambda x: x['timestamp']): ts = comp['timestamp']; display_ts_str = "Invalid Date";
            try:
            ...
            except Exception: pass; comp_options[f"{display_ts_str} (ID: {comp['id']})"] = comp['id']
            options_list = list(comp_options.keys()); ids_list = list(comp_options.values())
            if st.button("Select for Time Comparison", key=f"select_group_{idx}", use_container_width=True): st.session_state.selected_url_key_for_time_comp = url_key; st.session_state.time_comp_id1 = None; st.session_state.time_comp_id2 = None; st.session_state.df_time_comparison = pd.DataFrame(); st.rerun()
            if st.session_state.selected_url_key_for_time_comp == url_key:
                st.caption("Select two snapshots:"); current_idx1 = ids_list.index(st.session_state.time_comp_id1) if st.session_state.time_comp_id1 in ids_list else 0; current_idx2 = ids_list.index(st.session_state.time_comp_id2) if st.session_state.time_comp_id2 in ids_list else min(1, len(ids_list)-1) if len(ids_list) > 1 else 0
            ...
            for comp_meta in comps_list:
                 ...
                 except: pass; display_label = f"{display_ts_str} (ID: {comp_id})"; is_selected = str(comp_id) == viewing_saved_id_check; t_col1, t_col2 = st.columns([0.85, 0.15]) # Use checked var
                 with t_col1: button_type = "primary" if is_selected else "secondary";
...
def display_all_results(df_ounass, df_levelshoes, df_comparison_sorted, stats_title_prefix="Overall Statistics", is_saved_view=False, saved_meta=None):
    ...
    if is_saved_view and saved_meta:
         ...
         except: pass; st.subheader(f"Viewing Saved Comparison ({display_ts_str})"); st.caption(f"Ounass URL: `{saved_meta.get('ounass_url', 'N/A')}`"); st.caption(f"Level Shoes URL: `{saved_meta.get('levelshoes_url', 'N/A')}`")
    else: url_for_stats = st.session_state.get('processed_ounass_url') or st.session_state.get('ounass_url_input');
    ...
    if not is_saved_view and df_comparison_sorted is not None and not df_comparison_sorted.empty:
        ...
        with stat_save_col: st.write("");
        if st.button("💾 Save", key="save_live_comp_confirm", help="Save current comparison results", use_container_width=True): oun_url = st.session_state.get('processed_ounass_url', st.session_state.get('ounass_url_input','')); ls_url = st.session_state.get('levelshoes_url_input', ''); df_save = st.session_state.df_comparison_sorted;
        if save_comparison(oun_url, ls_url, df_save): st.success(f"Comparison saved! ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})"); st.session_state.confirm_delete_id = None; st.rerun()
    else: st.subheader(stats_title);
    df_o_safe = df_ounass if df_ounass is not None and not df_ounass.empty else pd.DataFrame(); df_l_safe = df_levelshoes if df_levelshoes is not None and not df_levelshoes.empty else pd.DataFrame(); df_c_safe = df_comparison_sorted if df_comparison_sorted is not None and not df_comparison_sorted.empty else pd.DataFrame();
    ...
    if not df_c_safe.empty and 'Ounass_Count' in df_c_safe.columns and 'LevelShoes_Count' in df_c_safe.columns:
        ...
        common_brands_count = len(df_c_safe[(df_c_safe['Ounass_Count'] > 0) & (df_c_safe['LevelShoes_Count'] > 0)]); ounass_only_count = len(df_c_safe[(df_c_safe['Ounass_Count'] > 0) & (df_c_safe['LevelShoes_Count'] == 0)]); levelshoes_only_count = len(df_c_safe[(df_c_safe['Ounass_Count'] == 0) & (df_c_safe['LevelShoes_Count'] > 0)]);
    ...
    if not is_saved_view: col1, col2 = st.columns(2);
    with col1: st.subheader("Ounass Results");
    ...
    elif not st.session_state.get('df_ounass_processed', False) and not is_saved_view: st.info("Enter Ounass URL and click 'Process URLs'.");
    with col2: st.subheader("Level Shoes Results");
    ...
    elif not st.session_state.get('df_levelshoes_processed', False) and not is_saved_view: st.info("Enter Level Shoes URL and click 'Process URLs'.");
    if df_comparison_sorted is not None and not df_comparison_sorted.empty:
        if not is_saved_view: st.markdown("---"); st.subheader("Ounass vs Level Shoes Brand Comparison"); df_display = df_comparison_sorted.copy(); df_display.index += 1; display_cols = ['Display_Brand', 'Ounass_Count', 'LevelShoes_Count', 'Difference']; missing_cols = [col for col in display_cols if col not in df_display.columns];
        if missing_cols: st.warning(f"Comparison table missing columns: {', '.join(missing_cols)}"); st.dataframe(df_display, height=500, use_container_width=True);
        else: st.dataframe(df_display[display_cols], height=500, use_container_width=True); st.markdown("---"); st.subheader("Visual Comparison"); viz_col1, viz_col2 = st.columns(2);
        with viz_col1: st.write("**Brand Overlap**"); pie_data = pd.DataFrame({'Category': ['Common Brands', 'Ounass Only', 'Level Shoes Only'],'Count': [common_brands_count, ounass_only_count, levelshoes_only_count]}); pie_data = pie_data[pie_data['Count'] > 0];
        ...
        with viz_col2: st.write("**Top 10 Largest Differences (Count)**");
        if 'Difference' in df_comparison_sorted.columns and 'Display_Brand' in df_comparison_sorted.columns: top_pos = df_comparison_sorted[df_comparison_sorted['Difference'] > 0].nlargest(5, 'Difference'); top_neg = df_comparison_sorted[df_comparison_sorted['Difference'] < 0].nsmallest(5, 'Difference'); top_diff = pd.concat([top_pos, top_neg]).sort_values('Difference', ascending=False);
        ...
        else: st.info("Difference data unavailable."); st.markdown("---"); st.subheader("Top 15 Brands Comparison (Total Products)");
        if not df_comparison_sorted.empty and all(c in df_comparison_sorted.columns for c in ['Display_Brand', 'Ounass_Count', 'LevelShoes_Count']): df_comp_copy = df_comparison_sorted.copy(); df_comp_copy['Total_Count'] = df_comp_copy['Ounass_Count'] + df_comp_copy['LevelShoes_Count']; top_n = 15; top_brands = df_comp_copy.nlargest(top_n, 'Total_Count');
        if not top_brands.empty: melted = top_brands.melt(id_vars='Display_Brand', value_vars=['Ounass_Count', 'LevelShoes_Count'], var_name='Website', value_name='Product Count'); melted['Website'] = melted['Website'].str.replace('_Count', '').str.replace('LevelShoes','Level Shoes'); fig_top = px.bar(melted, x='Display_Brand', y='Product Count', color='Website', barmode='group', title=f"Top {top_n} Brands by Total Products", labels={'Display_Brand': 'Brand'}, category_orders={"Display_Brand": top_brands['Display_Brand'].tolist()}); fig_top.update_layout(xaxis_title=None); st.plotly_chart(fig_top, use_container_width=True);
        else: st.info(f"Not enough data for Top {top_n} chart.");
        else: st.info(f"Comparison data unavailable for Top {top_n} chart."); st.markdown("---"); col_comp1, col_comp2 = st.columns(2); req_cols_exist = all(c in df_comparison_sorted.columns for c in ['Display_Brand', 'Ounass_Count', 'LevelShoes_Count', 'Difference']);
        with col_comp1: st.subheader("Brands in Ounass Only");
        ...
        elif req_cols_exist: st.info("No unique Ounass brands found."); else: st.info("Data unavailable.");
        with col_comp2: st.subheader("Brands in Level Shoes Only");
        ...
        elif req_cols_exist: st.info("No unique Level Shoes brands found."); else: st.info("Data unavailable."); st.markdown("---"); col_comp3, col_comp4 = st.columns(2);
        with col_comp3: st.subheader("Common Brands: Ounass > Level Shoes");
        ...
        elif req_cols_exist: st.info("No common brands where Ounass > LS."); else: st.info("Data unavailable.");
        with col_comp4: st.subheader("Common Brands: Level Shoes > Ounass");
        ...
        if req_cols_exist and not df_f.empty: df_d = df_f[['Display_Brand', 'Ounass_Count', 'LevelShoes_Count', 'Difference']].reset_index(drop=True); df_d.index += 1; st.dataframe(df_d, height=400, use_container_width=True);
        elif req_cols_exist: st.info("No common brands where LS > Ounass."); else: st.info("Data unavailable."); st.markdown("---"); csv_buffer_comparison = io.StringIO(); dl_cols = ['Display_Brand', 'Ounass_Count', 'LevelShoes_Count', 'Difference'];
        if req_cols_exist: df_comparison_sorted[dl_cols].to_csv(csv_buffer_comparison, index=False, encoding='utf-8'); csv_buffer_comparison.seek(0); download_label = f"Download {'Saved' if is_saved_view else 'Current'} Comparison (CSV)"; view_id_part = saved_meta['id'] if is_saved_view and saved_meta else 'live'; download_key = f"comp_dl_button_{'saved' if is_saved_view else 'live'}_{view_id_part}"; filename_desc = f"{detected_gender or 'All'}_{detected_category or 'All'}".replace(' > ','-').replace(' ','_').lower(); download_filename = f"brand_comparison_{filename_desc}_{view_id_part}.csv".replace('?_?', 'all_all'); st.download_button(download_label, csv_buffer_comparison.getvalue(), download_filename, 'text/csv', key=download_key);
        else: st.warning("Could not generate download: missing columns.");
    elif process_button and not is_saved_view: st.markdown("---"); st.warning("Comparison could not be generated. Check individual results.");
...
def display_time_comparison_results(df_time_comp, meta1, meta2):
    ...
    try: ts1 = meta1.get('timestamp'); ts2 = meta2.get('timestamp');
    ...
    def display_change_df(df_change, category_name, count_col_t1, count_col_t2, change_col, sort_col, sort_ascending, rename_map):
        if not df_change.empty: st.write(f"{category_name} ({len(df_change)}):"); display_cols = ['Display_Brand'];
        if count_col_t1: display_cols.append(count_col_t1);
        if count_col_t2: display_cols.append(count_col_t2);
        if change_col: display_cols.append(change_col); df = df_change[display_cols].rename(columns=rename_map).sort_values(sort_col, ascending=sort_ascending).reset_index(drop=True); df.index += 1; st.dataframe(df, height=height, use_container_width=True); return True
        return False
    with tc_col1: st.write("**Ounass Changes**"); displayed_any_o = False; rename_new_drop = {'Ounass_Count_T1':'Was', 'Ounass_Count_T2':'Now'}; rename_inc_dec = {'Ounass_Count_T1':'Was', 'Ounass_Count_T2':'Now', 'Ounass_Change':'Change'};
    ...
    if display_change_df(inc_o, "Increased", 'Ounass_Count_T1', 'Ounass_Count_T2', 'Ounass_Change', 'Change', False, rename_inc_dec): displayed_any_o = True; dec_o_display = dec_o[dec_o['Ounass_Count_T2'] > 0];
    ...
    with tc_col2: st.write("**Level Shoes Changes**"); displayed_any_l = False; rename_new_drop = {'LevelShoes_Count_T1':'Was', 'LevelShoes_Count_T2':'Now'}; rename_inc_dec = {'LevelShoes_Count_T1':'Was', 'LevelShoes_Count_T2':'Now', 'LevelShoes_Change':'Change'};
    ...
    if display_change_df(inc_l, "Increased", 'LevelShoes_Count_T1', 'LevelShoes_Count_T2', 'LevelShoes_Change', 'Change', False, rename_inc_dec): displayed_any_l = True; dec_l_display = dec_l[dec_l['LevelShoes_Count_T2'] > 0];
    ...
    if not displayed_any_l: st.info("No significant changes for Level Shoes."); st.markdown("---"); csv_buffer = io.StringIO();
    if all(col in df_time_comp.columns for col in req_time_cols): df_time_comp[req_time_cols].to_csv(csv_buffer, index=False, encoding='utf-8'); csv_buffer.seek(0); st.download_button(label=f"Download Time Comparison ({ts1_str} vs {ts2_str})", data=csv_buffer.getvalue(), file_name=f"time_comparison_{id1}_vs_{id2}.csv", mime='text/csv', key='time_comp_dl_button');
...
if confirm_id:
    ...
    if saved_meta and saved_df is not None:
        # Display saved results (URL inputs are hidden by the logic at the top)
    ...
    if process_button: # Check if button (defined above conditionally) was clicked
        ...
        if st.session_state.ounass_url_input:
            with st.spinner("Processing Ounass URL..."):
                ...
                if ounass_html_content: st.session_state.ounass_data = process_ounass_html(ounass_html_content)
                if st.session_state.ounass_data:
                    try: st.session_state.df_ounass = pd.DataFrame(st.session_state.ounass_data);
                    if not st.session_state.df_ounass.empty: st.session_state.df_ounass['Brand_Cleaned'] = st.session_state.df_ounass['Brand'].apply(clean_brand_name); st.session_state.df_ounass_processed = True
                    else: st.warning("Ounass data extracted empty.")
        ...
        if st.session_state.levelshoes_url_input:
             with st.spinner("Processing Level Shoes URL..."):
                ...
                if levelshoes_html_content: st.session_state.levelshoes_data = process_levelshoes_html(levelshoes_html_content)
                if st.session_state.levelshoes_data:
                    try: st.session_state.df_levelshoes = pd.DataFrame(st.session_state.levelshoes_data);
                    if not st.session_state.df_levelshoes.empty: st.session_state.df_levelshoes['Brand_Cleaned'] = st.session_state.df_levelshoes['Brand'].apply(clean_brand_name); st.session_state.df_levelshoes_processed = True
                    else: st.warning("Level Shoes data extracted empty.")
        ...
        if st.session_state.df_ounass_processed and st.session_state.df_levelshoes_processed:
            with st.spinner("Generating comparison..."):
             ...
             if process_button: # Show warnings only if button was pressed
                 if st.session_state.ounass_url_input and not st.session_state.df_ounass_processed: st.warning("Could not process Ounass. Comparison not generated.")
                 if st.session_state.levelshoes_url_input and not st.session_state.df_levelshoes_processed: st.warning("Could not process Level Shoes. Comparison not generated.")
